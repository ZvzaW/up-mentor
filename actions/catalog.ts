...
export async function getCatalogTrainers(filters?: PublicTrainersCatalogFilters) {
    ...
    try {
      type TrainerFindManyArgs = NonNullable<Parameters<typeof prisma.trainer.findMany>[0]>;
      ...
      const data = trainers.map((trainer) => {
        const averageRate =
          trainer.opinion.length > 0
            ? Number(
                (
                  trainer.opinion.reduce((sum, opinion) => sum + opinion.rate, 0) /
        ...
        return {
          ...
          workplaces: Array.from(new Set(trainer.workplace.map((workplace) => workplace.city))),
          averageRate,
        };
      });
